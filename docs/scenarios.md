...
Scenarios allow you to define complex, stateful sequences of responses for specific endpoints. This is the core of `flakymock`'s fault injection capability.
...
Set `SCENARIO_ROOT_DIR` (default: `.`). On startup, FlakyMock loads scenarios from:

- `scenarios.yaml` (legacy bootstrap file)
- `mappings/*.json` and `mappings/*.yaml` under `SCENARIO_ROOT_DIR`

Save in-memory scenarios to disk:

```bash
curl -X POST http://localhost:8080/api/control/persist-scenarios
```

List, update, and delete via `GET /scenarios`, `PUT /scenario`, `DELETE /scenarios`.
...
Trigger scenarios only when specific conditions are met.

**1. Header & Query Matching**
```yaml
- path: /api/search
  method: GET
  matches:
    ...
    headers:
      X-User-Type: "admin"   # AND header is admin
  responses:
    - status: 200
      body: "Admin Search Results"
```

**2. Smart JSON Matching (New)**
The server understands JSON. It matches if the request body is legally equivalent to the config, regardless of key order or whitespace.
...
*Matches:* `{"name": "foo", "id": 1}` ✅
*Matches:* `{"id": 1, "name": "foo" }` ✅

**3. Regex Matching**
To use regex, wrap the pattern in forward slashes `/`.
