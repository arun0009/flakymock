...
type Scenario struct {
	...
	Matches        MatchConfig          `yaml:"matches" json:"matches"`
	...
	CBState        *CircuitBreakerState `yaml:"-" json:"-"` // Runtime state
	Index          int32                `yaml:"-" json:"-"` // Current response index (atomic operations)
}
...
type Response struct {
	...
	DelayRange  string            `yaml:"delayRange" json:"delayRange"` // e.g., "100ms-500ms"
	Body        JSONBody          `yaml:"body" json:"body"`
...
func LoadConfig(scenarioFile string) (Config, error) {
	...
	if root := os.Getenv("SCENARIO_ROOT_DIR"); root != "" {
		SetScenarioRootDir(root)
	}
	if persisted, err := LoadMappingsFromDir(ScenarioRootDir); err != nil {
		return Config{}, err
	} else {
		for i := range persisted {
			persisted[i].CBState = &CircuitBreakerState{State: "closed"}
			addScenarioLocked(&persisted[i])
		}
	}
...
func addScenarioLocked(s *Scenario) {
	...
	key := scenarioMapKey(s.Path, s.Method)
...
// ResetScenarios resets response indices and circuit breaker state.
// Empty path or method resets all scenarios; otherwise filters by template path and method.
func ResetScenarios(pathFilter, methodFilter string) int {
	reset := 0
	scenarios.Range(func(key, value interface{}) bool {
		scenarioKey := key.(string)
		parts := strings.Split(scenarioKey, "_")
		if len(parts) != 2 {
			return true
		}
		tmplPath, method := parts[0], parts[1]
		if pathFilter != "" && tmplPath != pathFilter {
			return true
		}
		...
		for _, s := range value.([]*Scenario) {
			...
			if s.CBState != nil {
				...
				s.CBState.LastTransition = time.Time{}
				s.CBState.LastFailure = time.Time{}
				s.CBState.Mutex.Unlock()
			}
			reset++
		}
		return true
